...
public class GameActivity extends Activity {
	...
	public void onResume() {
		super.onResume();
		gameSurface.getGameData().open();
	}

	@Override
	public void onPause() {
		super.onPause();
		gameSurface.getGameData().close();
	}

	@Override
	public void onStop() {
		super.onStop();
		gameSurface.killSensor();
	...
	private class DialogHandler extends Handler {
		public void handleMessage(Message msg) {
            if (GameMessage.WIN.equals(msg.obj)) {
                if (!GameSurfaceView.getLevel().getInfo().getFinishedBool() || gameSurface.getRunTime() < GameSurfaceView.getLevel().getInfo().getTime()) {
                	GameSurfaceView.getLevel().getInfo().setTime(gameSurface.getRunTime());
                	GameSurfaceView.getLevel().getInfo().setCompleted(1);
                	gameSurface.getGameData().addLevelInfo(GameSurfaceView.getLevel().getInfo());
                }
                    ...
                    public void handleMessage(Message msg) {
                        if (GameMessage.NEXT.equals(msg.obj)) {
                            if (level == 21) {
                                dialog.cancel();
                                finish();
                                Logger.getLogger(GameActivity.class.getName()).warning("Not making it here");
